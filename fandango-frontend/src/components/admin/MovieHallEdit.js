...
class MovieHallEdit extends Component {
    ...
    constructor(props) {
        ...
        this.state={
            ...
            usedSelectedHallFromParams:false,
        }
    ...
    componentDidUpdate() {
        ...
        const { selectedMovieHall,usedSelectedHallFromParams } = this.state;
        const {movieHalls,loadingHalls}=this.props;
        const {params} = this.props.match;
        if(params.hallId && params.hallId!="" && !loadingHalls && !usedSelectedHallFromParams && Object.keys(selectedMovieHall).length===0){
            console.log("in")
            movieHalls.map((item)=>{
                if(item._id==params.hallId){
                    this.setState({usedSelectedHallFromParams:true,selectedMovieHall:item,newMovieHall:{...item}});
                }
            })
        }
    ...
    handleSubmit(e){
        ...
        if(Object.keys(newMovieHall).length===0){
            ...
            this.props.editHall(formData,newMovieHall._id);
    ...
    render() {
        ...
        const { selectedMovieHall,usedSelectedHallFromParams } = this.state;
        const { newMovieHall } = this.state;
        const {movieHalls,loadingHalls,movies,loadingMovies}=this.props;
                ...
                <Container>
                    ...
                    <Row>
                        <Col md={{size: 3, offset: 3}}>
                            <Select
                                name="movieHallSelect"
                                value={selectedMovieHall}
                    ...
                    <Row noGutters={true} style={{margin: 25}}>
                        ...
                        <Col sm="12" md={{size: 8, offset: 2}}>
                            <form onSubmit={this.handleSubmit}>
                                ...
                                <div className="form-group">
                                    <label htmlFor="name">Owner Password</label>
...
function mapStateToProps(state) {
    ...
    const {loadingHalls}=state.admin;
    const {movies}=state.admin;
    const {loadingMovies}=state.admin;
    return {
        user,
        movieHalls,
        movies,
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        editHall: (hall,id)=>dispatch(adminActions.editHall(hall,id)),
    };
}

export default withRouter(connect(mapStateToProps, mapDispatchToProps)(MovieHallEdit));